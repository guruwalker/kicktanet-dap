...
const { uploadMe } = useSupabaseMe(); // Changed to useSupabaseMe
...
const handleUpload = async () => {
  ...
  try {
    const uploadedMe = await uploadMe(
      selectedFiles.value,
      selectedComponent.value
    );

    if (!Array.isArray(uploadedMe) || uploadedMe.length === 0) {
      uploadError.value = "No files were uploaded. Please try again.";
      return;
    }

    for (const file of uploadedMe) {
      try {
        ...
        if (!response?.success) {
          console.error("Failed to save file to media_files table:", response?.message);
          toast.add({
            title: "Warning",
            description: `Failed to save the file to the media_files table.`,
            color: "yellow",
            id: "media-warning",
          });
        } else {
    ...
    <UCard>
      <template #header>
        <div class="flex justify-between items-center">
          <h2 class="text-lg font-semibold">Upload M&E Files</h2>
          <button
            @click="emit('close')"
