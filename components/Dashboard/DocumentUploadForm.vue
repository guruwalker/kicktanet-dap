...
const { uploadDocuments } = useSupabaseDocuments(); // Changed to useSupabaseDocuments
...
const handleUpload = async () => {
  ...
  try {
    ...
    if (!Array.isArray(uploadedDocuments) || uploadedDocuments.length === 0) {
      uploadError.value = "No documents were uploaded. Please try again.";
      return;
    }
    ...
    for (const document of uploadedDocuments) {
      try {
        ...
        if (!response?.success) {
          console.error(
            "Failed to save document to media_files table:",
            response?.message
          );
          toast.add({
            title: "Warning",
            description: `Failed to save the document to the media_files table.`,
            color: "yellow",
            id: "media-warning",
          });
        } else {
          toast.add({
            title: "Success",
            description: "Documents uploaded and saved successfully!",
            color: "green",
            id: "media-success",
          });
