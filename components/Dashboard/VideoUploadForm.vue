...
const { uploadVideos } = useSupabaseVideos(); // Changed to useSupabaseVideos
...
const handleFileChange = (event: Event) => {
  ...
  if (files && files.length > 0) {
    if (files.length > 2) {
      uploadError.value =
        "You can only upload a maximum of 2 videos at a time.";
      return;
    }
...
const handleUpload = async () => {
  ...
  try {
    // Upload videos to Supabase and get their URLs
    ...
    if (!Array.isArray(uploadedVideos) || uploadedVideos.length === 0) {
      uploadError.value = "No videos were uploaded. Please try again.";
      return;
    }
    ...
    for (const video of uploadedVideos) {
      try {
        ...
        if (!response?.success) {
          console.error(
            "Failed to save video to media_files table:",
            response?.message
          );
          toast.add({
            title: "Warning",
            description: `Failed to save the video to the media_files table.`,
            color: "yellow",
            id: "media-warning",
          });
        } else {
          toast.add({
            title: "Success",
            description: "Videos uploaded and saved successfully!",
            color: "green",
            id: "media-success",
          });
